...
print('Filtering input point cloud...', output + '.laz')
...
pipeline_raster['pipeline'].append({ 'type': 'writers.raster', 'filename': output + '.tmp.tif', 'gdalopts': 'NUM_THREADS=ALL_CPUS,COMPRESS=DEFLATE' })
...
#newraster = gdal.GetDriverByName('COG').CreateCopy(output, raster, options = ['COMPRESS=DEFLATE', 'LEVEL=9', 'NUM_THREADS=ALL_CPUS'])
#newraster = None
