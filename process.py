...
for y in (ypos-size, ypos, ypos+size):
    for x in (xpos-size, xpos, xpos+size):
        ...
        path = os.path.join(topdir, '*', '*', '*_' + str(y) + '_' + str(x) + '_' + str(size) + '.laz')
        result = glob.glob(path)
        if len(result) > 1:
            print("Error: Too many tiles matching glob pattern", path)
            print(result)
            exit(1)
        if len(result) == 0:
            print("Warning: Missing tile with glob pattern", path)
        if len(result) > 0:
...
print('Rasterizing small...', output)

pipeline_raster = {'pipeline': []}
pipeline_raster['pipeline'].append(output + '.laz')
pipeline_raster['pipeline'].append({ 'type': 'filters.delaunay' })
pipeline_raster['pipeline'].append({ 'type': 'filters.faceraster', 'resolution': 25.0, 'origin_x': xpos*100, 'origin_y': ypos*100, 'width': size*4, 'height': size*4 })
pipeline_raster['pipeline'].append({ 'type': 'writers.raster', 'filename': output + '.small.tif', 'gdalopts': 'NUM_THREADS=ALL_CPUS,TILED=YES,COMPRESS=ZSTD' })
